            ...
            <div class="col-sm-9 col-md-9">
                ...
                <g:if test="${grailsApplication.config.useDownloadPlugin?.toBoolean()}">
                    <div id="downloads" class="btn btn-primary pull-right">
                        <alatag:download searchResults="${sr}" searchRequestParams="${searchRequestParams}" qualityCategories="${qualityCategories}" class="tooltips newDownload" title="Download all ${g.formatNumber(number: sr.totalRecords, format: "#,###,###")} records">
                            ...
                            <g:each var="items" in="${sr.activeFacetObj}">
                                <g:if test="${items.key}">
                                    <g:each var="item" in="${items.value}">
                                        ...
                                        <alatag:currentFilterItem key="${items.key}" value="${item}" facetValue="${item.value}" cssClass="btn btn-default btn-xs" cssColor="${UserFQColors[item.value]}" title="${userFqInteractDQNames[item.value]}" addCloseBtn="${true}"/>
                                    </g:each>
                                </g:if>
                            </g:each>
